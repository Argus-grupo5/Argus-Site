...
function cadastrarEmpresa(nome, telefone, senha, cnpj, razao, fkEndereco) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", nome, telefone, senha, cnpj, razao, fkEndereco);
    // Insira exatamente a query do banco aqui, lembrando da nomenclatura exata nos valores
    //  e na ordem de inserção dos dados.
    var instrucaoSql = `
        INSERT INTO empresa (nome_fantasia, telefone, senha, razao_social, cnpj,  fkEndereco) VALUES ('${nome}', '${telefone}', SHA2('${senha}', 512), '${razao}', '${cnpj}', ${fkEndereco});
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
